 ...
 * fills because both vendors' guidelines forbid recolouring them. Discord's
 * single-colour Clyde mark stays monochrome (`currentColor` → white on the
 * Blurple button), which is one of its sanctioned variants. Artwork: Simple
 * Icons (CC0) for GitHub/ORCID/Discord; Google's and Microsoft's own brand
 * assets for their marks. The GitHub, ORCID, Discord, Google, and Microsoft
...
/** Discord "Clyde" mark. Single-colour, so it inherits the button's text
 *  colour via `fill="currentColor"` (white on the Blurple button). */
export function DiscordMark({ className = 'h-5 w-5' }: MarkProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className={className}>
