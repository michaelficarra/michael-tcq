...
describe('MeetingPage tab–hash sync', () => {
  ...
  describe('tab change history entries', () => {
    /** Attach (or refresh) a spy on history.pushState/replaceState and start
     * with a clean call log — vi.spyOn returns the same instance on repeat
     * calls and accumulates calls across tests, so we clear explicitly. */
    ...
    it('uses pushState (not replaceState) so each tab click is a history entry', () => {
      renderMeetingPage();
      const { pushSpy, replaceSpy } = spyOnHistory();

      fireEvent.click(screen.getByRole('tab', { name: 'Agenda' }));

      expect(pushSpy).toHaveBeenCalledTimes(1);
      expect(pushSpy).toHaveBeenCalledWith(null, '', '#agenda');
      expect(replaceSpy).not.toHaveBeenCalled();
    });

    it('pushes a separate history entry for each successive tab click', () => {
      renderMeetingPage();
      const { pushSpy } = spyOnHistory();

      fireEvent.click(screen.getByRole('tab', { name: 'Log' }));
      fireEvent.click(screen.getByRole('tab', { name: 'Help' }));
      fireEvent.click(screen.getByRole('tab', { name: 'Queue' }));

      expect(pushSpy.mock.calls.map((c) => c[2])).toEqual(['#log', '#help', '#queue']);
    });
    ...
    it('does not push a history entry when the hash changes externally (browser back/forward)', () => {
      renderMeetingPage('#agenda');
      const { pushSpy } = spyOnHistory();
    ...
    it('simulating back-navigation after tab clicks restores the previous tab', () => {
      // Mimics: user clicks Agenda → Log, then presses browser back. The back
      // press surfaces in the test as a hashchange event reverting to the
      // previous hash. The active tab must follow.
      renderMeetingPage();
