...
export interface User {
   ...
   * stored on the meeting itself. Omitted entirely (rather than set to
   * `false`) for non-premium users to keep state and delta payloads
   * small; clients MUST treat absence as `false`. Gates premium-tier
   * features generally, not just the animated queue-entry border that
   * was the first such feature.
   */
  isPremium?: boolean;
}
