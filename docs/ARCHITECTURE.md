...
│           ├── auth/            # AuthenticationProvider interface, registry, GitHub + ORCID + Discord + Google + Microsoft impls
...
### Copy confirmation: `CopyButton`

The copy-to-clipboard controls (Copy Queue, Copy Results) share one `components/CopyButton` that owns both the `navigator.clipboard.writeText` call and a transient "Copied" confirmation raised on success. That confirmation is another `popover="manual"` element on the same foundation as the toasts — promoted to the top layer on the same callback-ref → state → effect → `showPopover()` pattern, so it escapes any clipping ancestor without a portal — but anchored to its own button rather than the viewport corner: it measures the button with `getBoundingClientRect` and centres itself just above via an inline fixed `top`/`left` plus the `.tcq-copied-tooltip` transform (the same JS-positioning choice as the dropdowns). It auto-hides on a short timer (and on scroll, since it doesn't re-anchor), and reuses the toast's `role="status"`/`role="alert"` split so success and the denied-clipboard fallback ("Copy failed") are announced to assistive tech. Call sites pass a `getText` thunk evaluated at click time; the component is the single place clipboard feedback lives, so new copy buttons get it for free.
...
When an admin adds or removes a premium entry, the server scans the in-memory meetings map and re-emits a full `state` snapshot to every meeting room containing a user that matches the changed reference. Matching is by canonical user reference (`buildUserRefIndex` / `userMatchesIndex` in `@tcq/shared`): a `provider:accountId` key for any provider, or — for the backward-compatible bare-handle form — a GitHub handle. So a `google:…` or `orcid:…` premium entry re-emits to those users' rooms too, not just GitHub ones. Other rooms see no traffic. This rides on the existing `emitFullState` path — `decorateMeetingForClient` already re-runs `stampPremium` from the manager on every emit, so the badge / glow flips on or off for connected participants without a refresh.
...
Authentication is structured around an `AuthenticationProvider` interface (`packages/server/src/auth/provider.ts`) so that multiple OAuth providers can coexist. Five are implemented — GitHub (`auth/github.ts`), ORCID (`auth/orcid.ts`), Discord (`auth/discord.ts`), Google (`auth/google.ts`), and Microsoft (`auth/microsoft.ts`) — and the abstraction generalises to further OAuth 2.0 / OIDC providers. The set of enabled providers is the registry in `auth/registry.ts` (order = login-button order: GitHub, then ORCID, then Discord, then Google, then Microsoft); a provider is "enabled" simply when its credentials are present in the environment. When none are enabled **and** the process is not running in production, the server runs in mock-auth mode (`isMockAuthEnabled()` in `mockAuth.ts`). The production guard is deliberate: a production deploy that is missing its OAuth credentials must fail closed (return 401s, offer no login) rather than silently auto-logging every visitor in as the admin mock user. The startup `auth_mode` log line reports `oauth`, `mock`, or `none` accordingly.

**ORCID** (`auth/orcid.ts`, `auth/orcidUser.ts`, `orcidApi.ts`) differs from GitHub in instructive ways the interface absorbs cleanly: the **ORCID iD** is the `accountId` (and there is no handle, so the badge shows the iD); login uses the `/authenticate` scope, whose token response already carries the iD and public display name, so basic sign-in needs no further call. ORCID provides **no avatar**, so one is synthesised via **Gravatar** — the SHA-256 of the researcher's public email (`d=identicon` returns the real photo when that email has a Gravatar, else a deterministic identicon); with no public email the iD is hashed instead. Reading the public email/employer and searching the registry use ORCID's **public API** (`pub.orcid.org`), which needs a client-credentials `/read-public` token that `orcidApi.ts` fetches and caches (refreshing on a 401). All public-API use is best-effort: login still succeeds (iD + name + identicon) if it fails. `ORCID_BASE_URL` points the whole flow at the ORCID sandbox for testing.

**Discord** (`auth/discord.ts`, `auth/discordUser.ts`) is a plain **OAuth 2.0** login (not OIDC), so it borrows GitHub's two-step shape rather than Google's id_token: the auth-code exchange returns an opaque access token, then a `GET /users/@me` bearer call fetches the profile. The Discord user **id** (a snowflake) is the `accountId`; the **username** — globally unique since Discord's 2023 discriminator removal — is the `handle`, so the badge tooltip shows `@username · discord` like a GitHub login (it is mutable, which is fine: a GitHub login is too, and the snowflake id stays the stable key). The display name is the `global_name` (falling back to the username). Only the `identify` scope is requested — the **email is never requested or stored**, so it can't leak via the tooltip. Unlike ORCID/Microsoft, Discord **supplies an avatar**: it is the CDN URL built from the id + avatar hash captured at login (`cdn.discordapp.com/avatars/<id>/<hash>.png`), with the silhouette as the no-avatar fallback — no Gravatar. Like Google, Discord has **no public lookup-by-id** with a user's own credentials, so it omits `resolveByAccountId` (referenced accounts resolve from the known-users cache, else fall back to the silhouette), and its directory is meeting-tier only.

**Google** (`auth/google.ts`, `auth/googleUser.ts`) is an **OpenID Connect** login. The authorization-code exchange returns an `id_token` (a JWT) directly from Google's token endpoint; the provider base64url-decodes the JWT payload but deliberately **does not verify its signature** — per Google's docs, verification is unnecessary for the auth-code flow when the id_token is received directly from the token endpoint over TLS (TLS authenticates Google). The OIDC `sub` claim is the `accountId` (Google has no handle); the `name`/`email`/`picture`/`hd` claims fill the rest, so basic sign-in needs no userinfo round-trip. The `email` is stored on the `User` so the badge tooltip can show `<email> · google` (the opaque numeric `sub` is meaningless to a human), falling back to the `sub` when no email is known. The avatar is the `picture` URL captured at login — there is no way to derive one from the `sub`, and crucially **Google has no public lookup-by-id**, so the provider omits `resolveByAccountId` entirely. Re-resolving a stored `google:<sub>` reference (a presenter listed before they join, a chair added by reference, a premium-list entry) therefore relies on the known-users cache below; an account never seen on this server degrades to the badge silhouette. Google's directory is meeting-tier only (no admin/Workspace scopes are requested), mirroring ORCID's local-match design.

**Microsoft** (`auth/microsoft.ts`, `auth/microsoftUser.ts`) is the same **OpenID Connect** shape as Google on the Microsoft identity platform (Entra ID) v2.0 endpoints: the auth-code exchange returns an `id_token` we decode without verifying the signature (received directly from the token endpoint over TLS — the shared decoder lives in `auth/idToken.ts`). The `sub` claim is the `accountId` (no handle); `name`/`preferred_username`/`email` fill the profile. As with Google, the email — here `email || preferred_username` (the UPN), since Microsoft often omits the explicit `email` claim — is stored on the `User` so the badge tooltip shows `<email> · microsoft` rather than the opaque `sub`. Microsoft's id_token carries **no avatar** (a real photo needs a Microsoft Graph binary call), so — like ORCID — the avatar is a **Gravatar** off the account email (real photo if registered, else an identicon); the `gravatarUrl` helper is shared via `auth/gravatar.ts`. Like Google, Microsoft has **no public lookup-by-id**, so it omits `resolveByAccountId` (referenced accounts resolve from the known-users cache, else fall back to a `sub`-seeded identicon), and its directory is meeting-tier only. The tenant defaults to `common` (work/school + personal accounts); `MICROSOFT_TENANT` restricts it to a single tenant.

**Known-users cache** (`packages/server/src/knownUsers.ts`). Because Google can't re-resolve a bare `sub`, a provider-agnostic process-wide `Map<UserKey, User>` records every real (non-placeholder) user the server has seen. It is populated at three points: the login callback (`recordUser` in `auth.ts`, covering users who log in but haven't joined a meeting), every meeting-user write (`ensureUser` in `meetings.ts`, the single central participant write — plus the chair-init path in `create()`), and boot (`MeetingManager.restore` iterates each restored meeting's participants, rebuilding the cache from the implicit persistence in meetings). It is consulted as a synchronous fallback by `resolveSelections` (`resolveUser.ts`, before the async provider round-trip) and by the premium-user resolver (`premiumDirectory.ts`, before the provider call, promoting any hit into that module's TTL cache). GitHub and ORCID benefit from the same fallback for free; the only post-restart gap is a user who logged in but never joined a meeting, who re-populates on their next request.
...
1. `GET /auth/:providerId` — Look up the provider and redirect to its authorization URL. The GitHub provider requests the `read:user` scope only; the consent screen therefore mentions only profile data, no org-membership permission. In mock-auth mode (non-production, no provider configured) any `:providerId` just clears the logged-out flag and the mock middleware repopulates a fake user; outside mock-auth mode an unknown/unconfigured `:providerId` 404s.
...
ORCID's directory (in `auth/orcid.ts`) instead searches the **global ORCID registry**: a query is matched first against ORCID participants already in the meeting, then — if it is a bare ORCID iD — resolved directly, otherwise passed to ORCID's `expanded-search` public-API endpoint (name/affiliation). Results dedupe by iD; the client shows an ORCID suggestion's iD only to disambiguate a duplicate (name, organisation) pair (since the iD is opaque). Registry results carry an identicon avatar; the real-email Gravatar is fetched when the pick is re-resolved via `resolveByAccountId`.
...
- **Provider capability** — the directory is exposed as the GitHub provider's optional `directory` capability (`DirectoryCapability` in `auth/provider.ts`). ORCID, Google, and Microsoft implement only a **meeting-tier** directory (matching participants already in the meeting); none has an org-membership directory like GitHub's, and a provider could omit the capability entirely.
