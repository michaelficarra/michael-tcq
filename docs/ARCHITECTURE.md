...
The `/api/users/autocomplete` endpoint backs every GitHub-username input. It answers from a server-wide in-memory directory (`packages/server/src/githubDirectory.ts`) with two caches: per-user `userOrgs` (the orgs each searcher _publicly_ belongs to) and per-org `orgMembers` (the _public_ members of each org, each enriched with display name and company). Both are filled on the user's behalf using their persisted OAuth access token.
