...
1. `GET /auth/github` — Redirect to `https://github.com/login/oauth/authorize` with client ID and requested scopes (`read:user read:org` — the latter so the autocomplete directory can enumerate the user's orgs and concealed members).
...
The `/api/users/autocomplete` endpoint backs every GitHub-username input. It answers from a server-wide in-memory directory (`packages/server/src/githubDirectory.ts`) with two caches: per-user `userOrgs` (the orgs each searcher belongs to) and per-org `orgMembers` (the members of each org, public and concealed). Both are filled on the user's behalf using their persisted OAuth access token.

- **Per-org enumeration uses the GitHub GraphQL API** — `organization(login).membersWithRole` returns `databaseId`, `login`, `name`, `company`, and `avatarUrl` in one paginated query, avoiding N follow-up `/users/{login}` REST calls just to populate display name and company.
- **ACL** — when answering an autocomplete request, only `orgMembers` for orgs in the searcher's `userOrgs` are consulted. A user can never see members of an org they don't themselves belong to, even if another logged-in user has populated that org's cache.
- **Three-tier search** — meeting users → caller's org members → global GitHub user search (`/search/users`, called on the user's behalf, only when tiers 1+2 don't fill the dropdown).
- **Reused for agenda import** — the import handler resolves each parsed presenter name against the same directory via `searchUsersLocal` / `resolvePresenterFromDirectory` (tiers 1+2 only; tier 3 is intentionally skipped) and binds the item to the real user only when there's exactly one match. This is why `searchUsersLocal` is split out from `searchUsers` — autocomplete and import share ranking semantics but not autocomplete's background cache-warm side-effect.
...
- **Strict at the write boundary, lenient on render** — `validateInlineMarkdown` is wired into the Zod schemas in `packages/shared/src/messages.ts`; submitting a heading, list, table, raw `<script>`, or `javascript:` URL is rejected with a specific reason. The renderer (`<InlineMarkdown>`) and the agenda import parser run inputs through `stripUnsupportedMarkdown` first, so legacy stored content and TC39 source markdown that uses constructs we don't render keep working — the disallowed nodes are dropped, the surviving inline content remains.
- **No `dangerouslySetInnerHTML`** — the renderer walks the parsed hast tree and emits React elements directly. Disallowed tags fall through to their inner text; disallowed attributes are silently filtered.
