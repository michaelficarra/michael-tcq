...
**Google** (`auth/google.ts`, `auth/googleUser.ts`) is an **OpenID Connect** login. The authorization-code exchange returns an `id_token` (a JWT) directly from Google's token endpoint; the provider base64url-decodes the JWT payload but deliberately **does not verify its signature** — per Google's docs, verification is unnecessary for the auth-code flow when the id_token is received directly from the token endpoint over TLS (TLS authenticates Google). The OIDC `sub` claim is the `accountId` (Google has no handle, so the badge shows `<sub> · google`); the `name`/`email`/`picture`/`hd` claims fill the rest, so basic sign-in needs no userinfo round-trip. The avatar is the `picture` URL captured at login — there is no way to derive one from the `sub`, and crucially **Google has no public lookup-by-id**, so the provider omits `resolveByAccountId` entirely. Re-resolving a stored `google:<sub>` reference (a presenter listed before they join, a chair added by reference, a premium-list entry) therefore relies on the known-users cache below; an account never seen on this server degrades to the badge silhouette. Google's directory is meeting-tier only (no admin/Workspace scopes are requested), mirroring ORCID's local-match design.

**Microsoft** (`auth/microsoft.ts`, `auth/microsoftUser.ts`) is the same **OpenID Connect** shape as Google on the Microsoft identity platform (Entra ID) v2.0 endpoints: the auth-code exchange returns an `id_token` we decode without verifying the signature (received directly from the token endpoint over TLS — the shared decoder lives in `auth/idToken.ts`). The `sub` claim is the `accountId` (no handle, so the badge shows `<sub> · microsoft`); `name`/`preferred_username`/`email` fill the profile. Microsoft's id_token carries **no avatar** (a real photo needs a Microsoft Graph binary call), so — like ORCID — the avatar is a **Gravatar** off the account email (real photo if registered, else an identicon); the `gravatarUrl` helper is shared via `auth/gravatar.ts`. Like Google, Microsoft has **no public lookup-by-id**, so it omits `resolveByAccountId` (referenced accounts resolve from the known-users cache, else fall back to a `sub`-seeded identicon), and its directory is meeting-tier only. The tenant defaults to `common` (work/school + personal accounts); `MICROSOFT_TENANT` restricts it to a single tenant.
