...
- Access an admin-only **Admin** tab on the home page — the active-meetings list, the diagnostics panel, and the Premium Users section (see **Admin Tools**).
- Soft-delete and restore any meeting (see **Admin Tools**).
...
Admin status is independent of the premium tier: a user can be either, both, or neither.

### Premium-tier users

The premium tier is a user-visible status that may gate additional features over time. It is managed by admins at runtime (see **Admin Tools → Premium Users**) and confers **no additional permissions or capabilities beyond a regular participant**. Premium status is independent of the admin role: a user can be either, both, or neither.

Two initial visual treatments mark a premium user, both visible to every participant in the meeting (not only to the premium user themselves):

- A verification-style badge is shown after the user's display name **everywhere their name appears** (see the canonical list under **Authentication → Display Names & Avatars**). The badge gently pulses in a heartbeat rhythm (disabled when the viewer prefers reduced motion) and surfaces a `TCQ Premium™` tooltip on hover.
...
Users authenticate via OAuth. The application supports multiple authentication providers and the login page shows one login button per configured provider, each in the provider's brand colour with its logo — currently **GitHub**, **ORCID**, **Discord**, **Google**, and **Microsoft** (in that order, each below the previous; every button reads "Sign in with …", and the Google and Microsoft buttons additionally use their mandated white variants).

When an unauthenticated user follows a deep link to a specific page (e.g. a meeting URL), completing login returns them to that page rather than the home page.

### Identity & Namespacing

A user's display name, account identifier, and organisation (as supplied by their provider) are used as their identity within the application. Their identity is namespaced by provider, so accounts from different providers never collide.

Each provider contributes an account identifier: GitHub users are identified by their login (handle), and Discord users by their globally-unique username (handle); **ORCID**, **Google**, and **Microsoft** users have no handle, so their provider account identifier (ORCID iD / Google account id / Microsoft account id) is the identifier.

### Display Names & Avatars

Wherever a user is shown — the "user badge", which is an avatar plus name (and organisation) — the badge appears in the queue entries, the current speaker, the current topic, agenda item presenters, the chair list, the navigation bar and user menu, the meeting log, and poll tooltips.

The **visible label is the person's display name**, never the raw account identifier: a GitHub user shows their display name (falling back to their login only when they have no display name set), a Discord user likewise falls back to their username, and an ORCID, Google, or Microsoft user shows their display name (falling back to their bare identifier only when the provider exposes no name for them).

Avatars are provider-specific: a Google user's avatar is the profile picture supplied by Google at sign-in, and a Discord user's is the avatar supplied by Discord; ORCID and Microsoft supply no avatar, so theirs comes from Gravatar (their photo when their email is registered with Gravatar, otherwise a generated identicon). Avatars are hidden gracefully if the image fails to load.

### Disambiguation Tooltip

Anywhere a user's name is shown, hovering it reveals a tooltip with the most recognisable identifier the provider offers, suffixed with the provider, so two people with the same display name can be told apart:

- a GitHub user's `@handle` or a Discord user's `@username` (e.g. `@nelly · discord`);
- a Google or Microsoft user's email (the UPN for Microsoft) (e.g. `someone@gmail.com · google`, `ada@contoso.com · microsoft`), since their account ids are opaque identifiers that mean nothing to a human — falling back to that id only when no email is known;
- and otherwise the bare account identifier (e.g. `0000-0002-1825-0097 · orcid`).

Note a Google or Microsoft user's email is therefore visible to other participants who hover their badge.

### Known-Users Cache

Because Google, Microsoft, and Discord (unlike GitHub and ORCID) offer no public way to look up an account by its identifier, the names and avatars of users who are _referenced_ in a meeting before they themselves sign in (a presenter listed on the agenda, a chair added by reference, a premium-list entry) are filled in from a **known-users cache**: any account that has been seen on the server before (it logged in, or joined any meeting) shows its real name and avatar wherever it's referenced. An account never seen on this server shows the generic placeholder (a silhouette, or for Microsoft an identicon) until it next appears. This applies to all providers but matters most for Google, Microsoft, and Discord.

### Mock Auth (Development)

In a non-production environment with no authentication provider configured, the server runs in mock auth mode with a fake user, allowing development without creating an OAuth App. In mock auth mode, a dev user-switcher in the navigation bar allows switching between different mock identities, and the login page shows a single, visually distinct "Enter dev mode" button rather than a provider button. Mock auth never activates in production: a production deployment that is missing its OAuth credentials fails closed (no login is possible) rather than silently granting everyone the admin mock identity.
...
The home page is organised into tabs: **Join Meeting**, an admin-only **Admin** tab (between **Join Meeting** and **Help**; see **Admin Tools**), and **Help**. The active tab is reflected in the URL fragment.

The **Join Meeting** tab shows two cards side by side:

- **Join Meeting** — a text input for the meeting ID and a "Join" button. Validates that the meeting exists before navigating to it.
- **New Meeting** — a "Start a New Meeting" button that creates a meeting with the current user as the sole chair and redirects to it (see **Creating a Meeting**).
...
The **Help** tab is available on the home page as well as inside a meeting (see **Navigation**).
...
If a user navigates to a non-existent meeting, a clear error page is shown with a link back to the home page (see **Error Handling**).

### Meeting Flow

1. A chair creates a meeting and shares the meeting ID with participants.
2. Participants join.
3. The chair clicks **Start Meeting** to advance to the first agenda item; its first presenter, if any, becomes the current speaker (see **Agenda → Presenters**).
...
### GitHub issue and PR link prettification
...
### Block markdown (prologue & epilogue)

The agenda prologue and epilogue (see **Agenda → Agenda Prologue and Epilogue**) accept a wider subset of markdown than the inline allowlist: paragraphs, headings (h1–h6), ordered and unordered lists (including nesting), thematic breaks (`---`), blockquotes, fenced code blocks, GFM tables, `<details>`/`<summary>` disclosure blocks, plus everything from the inline subset (bold, italic, code, links, autolinks, line breaks, allowed inline HTML). Every supported markdown construct's corresponding raw-HTML tag is also accepted, so authors can write `# heading` or `<h1>heading</h1>` interchangeably. Anything outside the allowlist — images, generic structural tags (`<div>`, `<span>`), XSS-vector tags/attributes (`<script>`, `<iframe>`, `<form>`, `on*` handlers, `style`, `class`, `id`, …), and `javascript:` / `data:` link schemes — is escaped to its literal source so the original markup appears as written; no element is created and nothing executes.

### External links

Any link that points to a destination outside the application — whether it appears in a user-generated markdown field (agenda item names, queue topics, conclusions, session names, poll topics) or in the application chrome itself (e.g. the hamburger menu's Report an issue entry) — opens in a new tab/window with `rel="noopener noreferrer"` and displays a small northeast-facing arrow indicator next to the link text so users can tell, before clicking, that the link leaves TCQ.
...
- **Conclusion** (optional) — free-form text recording what was decided or concluded for the item. It is set or edited only via the Next Agenda Item confirmation dialogue (see **Meeting Flow**); it is shown in the meeting log under the corresponding "Finished" entry and underneath the item in the agenda list once the item has been advanced past. If an item that already has a conclusion becomes the current item again (e.g. via reorder), the dialogue's textarea is pre-populated with the previously stored conclusion so the chair can edit or replace it.
...
Chairs can import an agenda from a **TC39 agenda URL** (a markdown document on the tc39/agendas GitHub repository) or from a **local JSON file**, and can **export** the current agenda to a JSON file. Imports append to the existing agenda.

#### URL import (TC39 agendas)

URL import optionally offers a **Slot into sessions with available capacity** checkbox; when enabled, each imported item (in source order) is inserted into the first existing session whose run still has enough remaining capacity, without reordering any existing agenda entries — items that fit no session are appended at the end. (Session slotting is URL import only; file import always appends in source order.)

The server fetches URL documents, parses numbered list items and markdown tables to extract item names, presenters, and estimated durations (TC39 agendas still label this column "timebox"; the parser maps it to the `duration` field).

#### Markdown parsing rules

- A presenter column or parenthetical may list multiple names separated by `,`, `&`, or whitespace-bounded `and` (or any mix), each becoming a distinct presenter; bare markdown links inside the parenthetical (slides/notes metadata) are ignored.
- If a row has no presenter information, the imported item is created with no presenters (the chair can edit one in afterwards).
- A numbered list item is imported only when its trailing parenthetical contains a duration (e.g. `(15m, Samina Husain)`, `(Chair, 10m)`, `(5m)`); a trailing parenthetical with no time (e.g. `(Jordan Harband)`, `(in insertion order)`) is treated as decorative metadata and the item is skipped, as are items with no trailing parenthetical at all.
- The parser still descends into any nested ordered list — children with their own timed parentheticals are imported as separate agenda items, and each child's name is prefixed with the chain of ancestor item names joined by `: ` (so "Project Editors' Reports → ECMA262 Status Updates (10m)" imports as `Project Editors' Reports: ECMA262 Status Updates`).
- Markdown formatting in item names is preserved.

#### File import (JSON schema)

File import accepts a JSON document that is a **top-level array** of entries — flat, mirroring the flat agenda data model (no object wrapper, no nesting). Each entry is discriminated by `type`: a session (`type: "session"`) or a topic (`type: "topic"`), and every entry requires a `name`. A session requires a `capacity` (positive integer minutes). A topic may optionally include `presenters` (an array of names) and a `duration` (positive integer minutes). There are no field aliases — unknown fields (including `timebox`, singular `presenter`, or a nested `topics` array) are rejected.

#### Presenter resolution

Each parsed presenter name (from both URL and file import) is run through the same directory the username autocomplete uses, restricted to the meeting-users and org-members tiers (the global GitHub search tier is intentionally skipped); when a name yields exactly one match, the imported item is bound to that real user, otherwise the name is recorded as-is without a GitHub association.

#### Export

Chairs can export the current agenda to a `.json` file ("Export to File", available whenever the agenda is non-empty). The exported document uses exactly the same flat format the file import accepts, so an exported file can be re-imported. Sessions serialise as `{ "type": "session", "name", "capacity" }` and topics as `{ "type": "topic", "name", "presenters"?, "duration"? }`, with the optional topic fields omitted when empty. Presenter names are the presenters' display names (free-text presenters round-trip verbatim; directory-resolved presenters export the display name the directory matched on).

### Username Autocomplete

Every place the app accepts a user — the agenda form's presenters input, the inline agenda-item edit form, the chair-add input, and the dev user-switcher — is backed by a fuzzy-match autocomplete dropdown that searches the **directory of the signed-in user's own provider**. For a **GitHub**-authenticated searcher the dropdown surfaces three deduped tiers in priority order:
...
For an **ORCID**-authenticated searcher the same dropdown instead searches the ORCID directory: tier 1 is still the ORCID users already known to the current meeting, and beyond that it queries the global ORCID **registry** by name and affiliation, plus a direct **ORCID-iD** lookup when the typed query is itself a well-formed iD. ORCID users have no handle, so each suggestion row shows the researcher's name and affiliation; the bare ORCID iD is surfaced only to disambiguate a duplicate (name, affiliation) pair (or when no name is available).
...
In the multi-select chip variant (agenda presenters), each committed token is rendered as a pill containing the user's avatar and login, with an inline button to remove it; pressing Backspace into an empty input removes the most recently added chip.
...
The Agenda tab shows the list of meeting chairs at the top, followed by a numbered list of agenda items. Each item shows its name (with inline markdown rendered), a badge per presenter (each with avatar, display name, and organisation), and the duration if set — labelled as an "Estimate" for current and future items and as a "Duration" for past items (see the **Duration** field above). Items where the current user is one of the presenters are visually distinguished with a coloured left border.
...
- **Per-item duration bar** — Every agenda item that sits inside a session's run and has a duration carries a split bar in the left indicator column (the same column the teal "your item" highlight uses). The bar is gray for the part of the duration that fits within capacity (growing from the top of the row) and red for the part that overflows (growing from the bottom of the row), with the two segments always summing to the row's height. An item that fits entirely shows a full-height gray bar; an item that sits entirely past the capacity line shows a full-height red bar; an item that straddles the line is split proportionally. Hovering the red segment surfaces an "Overflows by Xm" tooltip with the exact contribution. On the current row a darker shade (lighter shade in dark mode) overlays the elapsed portion, so the column reads as a fill gauge: the dark band grows top-down at one second per second, tracking gray → red as it crosses the capacity split, and clamps at the full row height once the estimate is exceeded. The current row also displays a live `M:SS` (or `H:MM:SS`) elapsed-time readout alongside the estimate.
...
- **Past sessions** — Both in-list overflow indicators (the overflow subheader and the per-item `(overflows Xm)` text) are shown only for **current** and **upcoming** sessions. Once a session is in the past — the meeting has advanced beyond every item in its run, or the meeting has concluded — they are hidden to reduce clutter, since the session header's `overflow Xm` count already summarises the excess. A session is _current_ when the current agenda item sits within its run and _upcoming_ when the current item precedes its run (or the meeting hasn't started). The session-header capacity/used/overflow line is unaffected and renders for every session.
- **Live elapsed time** — The current agenda row displays an `(elapsed …)` readout next to its estimate that disappears as soon as the chair advances past the item. Its format and per-second cadence are described under **Queue → Timers**.
...
Both fields accept the block-markdown subset described under **Markdown & Links**.
...
3. **Reply** — a response to the current topic being discussed (displayed in cyan); only available when there is a current topic
4. **New Topic** — a new line of discussion (displayed in blue)
...
- Topic description
- Speaker name, organisation, and avatar
...
The currently speaking person is displayed prominently, showing their avatar, name, organisation, and the topic/question they queued with. A count-up timer shows how long the current speaker has been speaking (see **Timers**). Only one person speaks at a time.

### Current Topic

When a speaker introduces a new topic, that topic becomes the "current topic" and is displayed in a dedicated section. A count-up timer shows how long the current topic has been under discussion (see **Timers**). The current topic determines whether the "Reply" queue entry type is available, and the reply form references the current topic by name.

### Timers

The Queue tab displays live count-up timers on three elements:

- **Current agenda item** — time since the agenda item started (this is the same value shown as the `(elapsed …)` readout on the current agenda row; see **Agenda → Sessions**). If the item has an estimate, the timer turns bold red when the estimate is exceeded, and is annotated with the projected end time:
...
The Log tab provides a chronological record of meeting events, giving participants a timeline of what has happened during the meeting. It is part of the persisted meeting state and updated for all clients in real time (see **Persistence**).
...
Admins access an admin-only **Admin** tab on the home page (between the **Join Meeting** and **Help** tabs), hidden from non-admin users. It contains the active-meetings list, the diagnostics panel, and the Premium Users section described below. The panels on this tab refresh every 10 seconds.

### Active Meetings List

The Admin tab shows a list of all active meetings, showing each meeting's ID (linked to that meeting), creation time, distinct participant count, and last connection time. The creation and last-connection times render as relative durations (e.g. "3 hr ago") that update live, with the full ISO timestamp shown on hover. While at least one client is currently connected, the last-connection cell instead reads `now (N)`, where N is the current active-connection count (inclusive of every connected socket, admin or otherwise) — the same live-activity format the **My Meetings** panel uses, minus the "active" label.
...
Notification bodies that draw on user-authored markdown fields (topics, agenda item names, conclusions) are shown as plain text — link syntax collapses to the link label and any other inline markdown is stripped — because the browser's Notifications API displays the body verbatim with no formatting support (see **Markdown & Links**).
...
This is distinct from the browser-native **Notifications** described above (which fire at the OS level, even when the tab is in the background) and from the bottom-left connection-status indicator (see **Real-Time Updates**).
...
A small connection status indicator is displayed in the bottom-left corner of the meeting page: green when connected to the server, red when disconnected. This helps participants know whether they are seeing live state. While connected, hovering the indicator reveals the current number of active connections to the meeting. (The bottom-right corner is occupied by the toast stack; see **Toasts**.)

If the deployed server version changes while a participant is in a meeting — for example after a redeploy — a banner appears at the top of the meeting page reading "A new version of TCQ is available. Reloading in N seconds…" with a countdown and an immediate "Reload now" button. The page reloads automatically after the countdown. The short grace period lets participants copy anything they were typing before the reload discards in-progress input. The check is scoped to the meeting page (the only surface that pins the participant to a specific server via a long-lived WebSocket); other pages reload naturally on next interaction.
...
- Non-fatal errors (e.g. "Only chairs can...", "The queue is closed") are shown as transient, dismissible toasts (see **Toasts**).
...
The meeting log is permanent and cannot be edited or deleted during the meeting's lifetime. It is available to all connected participants and updated in real time.
