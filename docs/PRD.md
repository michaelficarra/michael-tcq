...
Users authenticate via OAuth. The application supports multiple authentication providers and the login page shows one login button per configured provider, each in the provider's brand colour with its logo — currently **GitHub**, **ORCID**, **Discord**, **Google**, and **Microsoft** (in that order, each below the previous; every button reads "Sign in with …", and the Google and Microsoft buttons additionally use their mandated white variants). A user's display name, account identifier, and organisation (as supplied by their provider) are used as their identity within the application; their identity is namespaced by provider, so accounts from different providers never collide. When an unauthenticated user follows a deep link to a specific page (e.g. a meeting URL), completing login returns them to that page rather than the home page.

GitHub users are identified by their login (handle), and Discord users by their globally-unique username (handle); **ORCID**, **Google**, and **Microsoft** users have no handle, so their provider account identifier (ORCID iD / Google account id / Microsoft account id) is the identifier. A Google user's avatar is the profile picture supplied by Google at sign-in, and a Discord user's is the avatar supplied by Discord; ORCID and Microsoft supply no avatar, so theirs comes from Gravatar (their photo when their email is registered with Gravatar, otherwise a generated identicon). Wherever a user is shown (the "user badge" — avatar plus name, used in the queue, current speaker, agenda presenters, chair list, navigation bar, and log), the **visible label is the person's display name**, never the raw account identifier: a GitHub user shows their display name (falling back to their login only when they have no display name set), a Discord user likewise falls back to their username, and an ORCID, Google, or Microsoft user shows their display name (falling back to their bare identifier only when the provider exposes no name for them). Anywhere a user's name is shown, hovering it reveals a tooltip with the most recognisable identifier the provider offers, suffixed with the provider, so two people with the same display name can be told apart: a GitHub user's `@handle` or a Discord user's `@username` (e.g. `@nelly · discord`); a Google or Microsoft user's email (the UPN for Microsoft) (e.g. `someone@gmail.com · google`, `ada@contoso.com · microsoft`, since their account ids are opaque identifiers that mean nothing to a human — falling back to that id only when no email is known); and otherwise the bare account identifier (e.g. `0000-0002-1825-0097 · orcid`). Note a Google or Microsoft user's email is therefore visible to other participants who hover their badge.
...
- **Edit** — Chairs can edit a session's name and capacity inline. Clearing the capacity field removes the limit (used/remaining/overflow stats are hidden until a capacity is set again).
