...
Users authenticate via OAuth. The application supports multiple authentication providers and the login page shows one "Log in with …" button per configured provider; currently GitHub is the only implemented provider ("Log in with GitHub"). A user's display name, account identifier, and organisation (as supplied by their provider) are used as their identity within the application; their identity is namespaced by provider, so accounts from different providers never collide. When an unauthenticated user follows a deep link to a specific page (e.g. a meeting URL), completing login returns them to that page rather than the home page.

Anywhere a user's name is shown, hovering it reveals a tooltip with their account identifier and provider (e.g. `@alice · github`), so two people with the same display name — or the same handle on different providers — can be told apart.

When no authentication provider is configured, the server runs in mock auth mode with a fake user, allowing development without creating an OAuth App. In mock auth mode, a dev user-switcher in the navigation bar allows switching between different mock identities.
