...
Users authenticate via OAuth. The application supports multiple authentication providers and the login page shows one login button per configured provider, each in the provider's brand colour with its logo — currently **GitHub**, **ORCID**, and **Google** (in that order, ORCID below GitHub and Google below ORCID; the Google button uses its mandated "Sign in with Google" white variant, the others read "Log in with …"). A user's display name, account identifier, and organisation (as supplied by their provider) are used as their identity within the application; their identity is namespaced by provider, so accounts from different providers never collide. When an unauthenticated user follows a deep link to a specific page (e.g. a meeting URL), completing login returns them to that page rather than the home page.

GitHub users are identified by their login (handle); **ORCID** and **Google** users have no handle, so their ORCID iD / Google account id is the identifier. ORCID provides no avatar, so an ORCID user's avatar is supplied by Gravatar (their photo when their ORCID email is public and registered with Gravatar, otherwise a generated identicon); a Google user's avatar is the profile picture supplied by Google at sign-in. Wherever a user is shown (the "user badge" — avatar plus name, used in the queue, current speaker, agenda presenters, chair list, navigation bar, and log), the **visible label is the person's display name**, never the raw account identifier: a GitHub user shows their display name (falling back to their login only when they have no display name set), and an ORCID or Google user shows their display name (falling back to their bare identifier only when the provider exposes no name for them). Anywhere a user's name is shown, hovering it reveals a tooltip with their account identifier and provider (e.g. `@alice · github`, `0000-0002-1825-0097 · orcid`, or `<account-id> · google`) — the opaque identifier is surfaced on hover, the same way a GitHub user's handle is — so two people with the same display name can be told apart.

Because Google (unlike GitHub and ORCID) offers no public way to look up an account by its identifier, the names and avatars of users who are _referenced_ in a meeting before they themselves sign in (a presenter listed on the agenda, a chair added by reference, a premium-list entry) are filled in from a **known-users cache**: any account that has been seen on the server before (it logged in, or joined any meeting) shows its real name and avatar wherever it's referenced. An account never seen on this server shows the generic silhouette until it next appears. This applies to all providers but matters most for Google.
