...
- **`K_REVISION`** — Cloud Run injects this automatically (no configuration needed); it's the revision name (e.g. `tcq-00042-abc`) and changes on every deploy, including deploys that reuse the same image. `GET /api/version` includes it as `revision` in the JSON response (`Accept: application/json`). The client polls this endpoint every 30 seconds and reloads itself when the revision changes, so participants whose long-lived WebSocket is still pinned to the drained old revision after a redeploy don't sit on a stale instance indefinitely. Plain-text responses omit `revision` — only JSON callers see it, keeping the monitoring contract unchanged.
