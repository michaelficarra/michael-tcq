...
- **`--concurrency=1000`** — Maximum number of concurrent requests per instance, raised from the Cloud Run default of 80. Each connected client holds one long-lived WebSocket request open for the entire meeting, so the default 80 caps the service at ~80 simultaneous attendees regardless of CPU/memory headroom. 1000 is Cloud Run's maximum and exceeds the per-meeting attendee scale we target.
...
- **`K_REVISION`** — Cloud Run injects this automatically (no configuration needed); it's the revision name (e.g. `tcq-00042-abc`) and changes on every deploy, including deploys that reuse the same image. The server publishes it two ways: as `revision` in the JSON response from `GET /api/version` (`Accept: application/json`), and once per socket via the `server:revision` WebSocket event on join. The meeting-page client takes the WebSocket-reported revision as its baseline (that's the revision its long-lived connection is actually pinned to), then polls `/api/version` every 30 seconds and reloads when the polled revision diverges. Sourcing the baseline from the WebSocket avoids a race where the page's first HTTP request and its WebSocket handshake land on different revisions if a deploy slips between them, which would falsely flag the tab as stale. Plain-text responses omit `revision` — only JSON callers see it, keeping the monitoring contract unchanged.
