...
- `e2e/form-validation.spec.ts` — Playwright spec for interaction-deferred form validation: a required field stays neutral until interaction, shows its invalid state (and `aria-invalid`) only after being left empty, and clears on a valid value. Needs a real browser because jsdom implements neither the `:user-invalid` pseudo-class nor constraint-validation timing; the `useAriaInvalidSync` bridge logic itself is unit-tested in `packages/client/src/hooks/useAriaInvalidSync.test.tsx`.
