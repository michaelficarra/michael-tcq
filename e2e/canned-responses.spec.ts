/**
 * End-to-end tests for the canned-responses feature.
 *
 * Covers:
 * - The smiley dropdown beside the entry-type buttons.
 * - Clicking a canned entry adds a finished topic to the queue immediately,
...
test.describe('Canned responses', () => {
  test('clicking a canned response adds a finished topic to the queue (no edit step)', async ({ page }) => {
    ...
    const trigger = controls.getByRole('button', { name: 'Canned responses' });
    await expect(trigger).toBeVisible();

    await trigger.click();

    const menu = page.getByRole('menu', { name: 'Canned responses' });
  ...
  test('editor supports add, edit, and delete; dropdown reflects the changes', async ({ page }) => {
    ...
    // Open the dropdown, then click "Edit canned responses…" to deep-link
    // into the Preferences modal.
    await page.getByRole('button', { name: 'Canned responses' }).click();
    await page.getByRole('menuitem', { name: /Edit canned responses/ }).click();

    const dialog = page.getByRole('dialog', { name: 'Preferences' });
    await expect(dialog).toBeVisible();

    // Rows are listitems inside the "Canned responses" list. Index 0 is
    // the seeded default; subsequent rows are inserted at the end.
    const rows = dialog.getByRole('list', { name: 'Canned responses' }).getByRole('listitem');
    ...
    // Add a new canned response. The new row's input is auto-focused so
    // we can type into the focused element directly.
    await dialog.getByRole('button', { name: 'Add canned response' }).click();
    ...
    const rows2 = dialog2.getByRole('list', { name: 'Canned responses' }).getByRole('listitem');
    await rows2.nth(0).getByRole('button', { name: 'Delete canned response' }).click();
    await page.keyboard.press('Escape');

    // Reopen dropdown — only the added entry should remain.
    await page.getByRole('button', { name: 'Canned responses' }).click();
    const menu2 = page.getByRole('menu', { name: 'Canned responses' });
  ...
  test('per-user lists are isolated', async ({ page }) => {
    ...
    // shared localStorage, but the canned-responses key is keyed by ghid,
