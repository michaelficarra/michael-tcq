...
- **Admin dashboard** — admins (configured via `ADMIN_USERNAMES` env var) get a dedicated **Admin** tab on the home page (hidden from non-admin users) showing a list of all active meetings with connection statistics — and the ability to delete meetings. A diagnostics panel alongside the meetings list shows server uptime, memory use, deployed git SHA, aggregate participant/connection counts, and the most recent ERROR/CRITICAL log entries — both panels refresh every 10 seconds so operators can triage problems without leaving the page.
