...
- **Avatars** — user avatars are shown alongside names throughout the application: GitHub avatars for GitHub users, and Gravatar (keyed off the public ORCID email, with an identicon fallback) for ORCID users.
- **Username autocomplete** — every user input (agenda presenters, chair list, dev user-switcher) shows a fuzzy-match dropdown backed by the searcher's own provider directory. For GitHub it prioritises users already in the meeting, then members of the searcher's own GitHub organisations (including concealed members), then a fallback global GitHub user search using the searcher's OAuth credentials. For ORCID it searches meeting users plus the global ORCID registry by name and affiliation, and resolves a pasted ORCID iD directly. Matching is case-insensitive. Free-text fallback still works for presenters who don't have an account.
