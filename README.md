...
- **Preferences** — a Preferences modal (in the hamburger menu or via `,`) lets users enable/disable keyboard shortcuts, opt into browser notifications, and choose a theme. Settings persist to `localStorage`.
...
- **Dark mode** — a full dark colour palette across every view. Follows the operating system's `prefers-color-scheme` by default (switching live when the OS setting changes), and can be forced to Light or Dark via the Preferences modal — or set to Inverse System, which tracks the OS setting live but renders the opposite palette. Random picks a palette by coin flip on each page load, and Inverse Random renders the opposite of whatever Random drew.
