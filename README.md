...
- **Live timers** — the Queue tab shows count-up timers on the current agenda item, current topic, and current speaker. The agenda item timer turns bold red when the estimate is exceeded. Active polls also display a count-up timer showing elapsed time since the poll was started.
...
- **Admin dashboard** — admins (configured via `ADMIN_USERNAMES` env var) see a list of all active meetings on the home page with connection statistics, and can delete meetings. A diagnostics panel below the meetings list shows server uptime, memory use, deployed git SHA, aggregate participant/connection counts, and the most recent ERROR/CRITICAL log entries — refreshed every 10 seconds so operators can triage problems without leaving the page.
