...
TCQ is a real-time web application for managing structured discussions during agenda-driven meetings. Among other things, TCQ solves the common problems caused by first-come-first-served participations systems (such as hand raising) wherein some participants have something to add to the current topic of discussion and some participants want to begin a new topic. It does this by providing a shared queue where participants can indicate how they plan to participate, while chairs control the flow of the meeting.
...
- **Agenda import** — chairs can import an agenda from a URL to a markdown document (e.g. a TC39 meeting agenda on GitHub) or from a local `.json` file. Imports append to the existing agenda. URL import can optionally slot items into sessions that still have capacity; file import always appends in source order. The markdown parser extracts items from both numbered lists and markdown tables, preserving markdown formatting in item names.
...
**Agenda file import (JSON)** — use **Import from File** on the Agenda tab. The file must be a JSON array, or an object with an `entries` array. Each entry is a `session` (optionally with nested `topics`) or a standalone `topic`. `timebox` and `duration` are equivalent (minutes). Presenter fields are optional; a session without a timebox is created without a capacity limit.
...
[
  {
    ...
    "timebox": 90,
    "topics": [
      { "name": "Welcome", "presenter": "Chair", "timebox": 5 },
      { "name": "Secretary's report", "presenters": ["Samina Husain"], "timebox": 15 }
    ]
  },
  { "type": "topic", "name": "Temporal update", "presenter": "Philip Chimento", "timebox": 30 },
...
- **Avatars** — user avatars are shown alongside names throughout the application.
- **Username autocomplete** — every user input (agenda presenters, chair list, dev user-switcher) shows a fuzzy-match dropdown backed by the searcher's own provider directory. For GitHub it prioritises users already in the meeting, then public members of the searcher's own GitHub organisations, then a fallback global GitHub user search using the searcher's OAuth credentials. For ORCID it searches meeting users plus the global ORCID registry by name and affiliation, and resolves a pasted ORCID iD directly. For Google and Microsoft it matches against that provider's users already in the meeting (neither exposes a public directory).
...
- **Structured production logging** — every HTTP request, Socket.IO event, and periodic task emits a JSON line to stdout with a GCP `severity` value. HTTP entries use the `LogEntry.HttpRequest` shape so Cloud Logging renders method, URL, status, and latency as first-class fields. Authenticated entries include the acting user's provider, account id, and handle. Uncaught exceptions and unhandled promise rejections are logged at `CRITICAL` before the process exits so the instance can be recycled cleanly.
- **Mock auth mode** — in non-production environments with no OAuth provider configured, a built-in dev user-switcher allows testing with multiple identities without configuring GitHub OAuth. It's disabled in production, so a misconfigured deploy fails closed rather than auto-logging everyone in.
...
TCQ is written in strict mode TypeScript 6 throughout, organised as three npm workspaces. The client is a React 19 single-page app built with Vite 8, styled with Tailwind CSS 4, and uses React Router 7 for navigation. The server runs on Node.js 24 with Express 5 and pushes real-time updates over Socket.IO 4 (using MessagePack encoding), backed by express-session for login state. A shared workspace holds the Zod 4 schemas and the unified/remark/rehype markdown pipeline that both ends rely on. Authentication is via OAuth through a pluggable provider interface (GitHub, ORCID, Discord, Google, and Microsoft today). Persistent state lives in Google Cloud Firestore. Tests run under Vitest 4 (unit and integration) and Playwright (end-to-end), with ESLint 10 and Prettier 3 enforcing style and SVGO and oxipng optimising image assets.
